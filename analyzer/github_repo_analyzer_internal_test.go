...
func TestIndexOf(t *testing.T) {
    t.Parallel()
    s := []string{"a","b","c"}
    if indexOf(s, "b") != 1 { t.Fatalf("want 1") }
    if indexOf(s, "x") != -1 { t.Fatalf("want -1") }
}
...
func TestFetchGithubInfo_NoToken_SetsSkip(t *testing.T) {
    ...
    if len(infos) != 1 { t.Fatalf("want 1 info") }
    if !infos[0].Skip { t.Fatalf("expected Skip true when token missing") }
    if infos[0].SkipReason == "" { t.Fatalf("expected SkipReason set") }
}
